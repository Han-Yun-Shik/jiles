...
import { REGDATE_YMD_STR, WR_STATE_ARR, WR_JCATE_ARR, WR_ALLCATE_ARR } from "@/app/utils";
import { useSearchParams, useRouter } from "next/navigation";
import Pagination from "@/components/Pagination"; // 추가
import { downloadExcelFile } from "@/lib/sjlistexcel";
...
export default function Sjlist() {
  ...
  const fetchData = async (searchParamsObj?: {
    ...
    try {
      ...
      const res = await axios.get(`/api/wroute/sjlist?${params.toString()}`);
  ...
  const updateUrl = (newParams: URLSearchParams) => {
    router.replace(`/wadm/sjlist?${newParams.toString()}`);
  };
      ...
      <div className="d-flex bg-secondary-subtle p-3">
        <div className="w-100 bg-white p-4 mt-4">
          <div className="jil_adm_c_hdr">
            <div className="jil_adm_c_hdr_left">재능장학금</div>
