...
export default function Shlist() {
  ...
  const fetchData = async (searchParamsObj?: {
    ...
    try {
      ...
      const res = await axios.get(`/api/wroute/shlist?${params.toString()}`);
  ...
  const updateUrl = (newParams: URLSearchParams) => {
    router.replace(`/wadm/shlist?${newParams.toString()}`);
  };
              ...
              {data.map((item, index) => {
                ...
                return <tr key={index}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <Link href={`/wadm/shupdate/${item.wr_code}`} className="btn btn-sm btn-primary mx-1">수정/보기</Link>
