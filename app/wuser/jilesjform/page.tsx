...
export default function Jilesjform() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.wr_cate === "jcate1" && (!formData.wr_school)) { alert("고등학교 정보를 모두 입력해 주세요."); setIsSubmitting(false); return; }
    if (formData.wr_cate === "jcate2" && (!formData.wr_schoolcode || !formData.wr_school)) { alert("대학교 정보를 모두 입력해 주세요."); setIsSubmitting(false); return; }
